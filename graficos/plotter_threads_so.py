...
with open(
    'C:\\Users\\felip\\Downloads\\30.06.2023\\tcc\\_com_max-memory-java_e_flag_server\\tcc\\visualvmresults\\visual_vm_3rd_execution_15s.csv','r'
    ) as csvfile:
    lines = csv.reader(csvfile, delimiter=';')
    for row in lines:
        if row[0] == 'Virtual Threads':
            xvt.append(row[1])
            yvt.append(int(row[8].replace(".","")))

        
        if row[0] == 'Cached Threads':
            xct.append(row[1])
            yct.append(int(row[8].replace(".","")))

        
        if row[0] == 'Tomcat Threads':
            xtomcat.append(row[1])
            ytomcat.append(int(row[8].replace(".","")))

grafico.rc('lines', linewidth=2.0)
fig, ax = grafico.subplots()
grafico.axis(ymin=0,ymax=750)
grafico.yticks(range(0,750,50))
grafico.grid(True)
...
grafico.ylabel("Nº de threads do SO")
